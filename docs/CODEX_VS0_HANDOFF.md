# CODEX — VS0 HANDOFF
    ...
    TASK STATUS: ACCEPTED — VS0-T01, VS0-T01R, VS0-T02, VS0-T03. Spec §39 is the
                 ...
                 T04 must not write any SOURCE file under /tests/** (see STOP-02 below
                 for the two generated .uid sidecars that are the sole exception), and
                 a green exit code is NOT acceptance. Before writing anything, run the
                 pre-write gate in spec §41.4: origin/main MUST equal T04_BASE. If it
                 does not, STOP — do not rebase onto a newer main.
                 ...
                 STOP-02: the second attempt ALSO stopped correctly, before any commit,
                 on two conditions — and both were approved by the owner. Read spec §43
                 in full. (1) --import generates SIX .uid script sidecars; exactly those
                 six are now authorized BY EXACT PATH (§42.6 row 3). NO *.uid WILDCARD IS
                 AUTHORIZED, and the five under presentation/** and /tests/** do NOT
                 authorize touching their .gd source files. (2) The §41.8 verification
                 block was DEFECTIVE IN THE SPEC and could never pass; it is corrected to
                 @($root.testsuite | Where-Object { ... }) — the whole pipeline wrapped.
                 The same array-safe rule binds verify_runner_failure.ps1 (§41.10).
                 T04's write scope: /addons/gut/** /tools/test/** /docs/ENGINE.md
                 /icon.svg.import and the SIX exact .uid paths enumerated in §42.6 —
                 the last seven are GENERATED by the pinned engine's import run, never
                 authored. Expected task shape: 269 A / 1 M / 0 D / 0 R / 0 C = 270.
                 Any OTHER versioned path produced by --import is a STOP (§42.6).
                 T04_BASE MOVES AGAIN with the STOP-02 patch: take it from the post-merge
                 handoff and run the §41.4 gate. Do not reuse a remembered SHA.
...
6. No GUT release matches Godot 4.7 — **the engine pin wins.** For VS0-T04 specifically, the full
   ...
   after a successful import, GUT still reporting missing `class_name`s after preparation, **any
   generated versioned path outside the exact set authorised in spec §42.6**, or **a mandated
   verification block that cannot pass against a report you believe is correct**.
   **Do not improvise around a stop** — STOP-01 (spec §42) and STOP-02 (spec §43) are exactly what
   stopping correctly looks like: no commit, no workaround, a report. **STOP-02 in particular
   reported that the SPEC's own verifier was broken instead of editing it to go green. That was
   right. Do that again.**
